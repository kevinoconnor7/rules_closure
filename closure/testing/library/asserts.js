...
// Don't install Closure fail if Jasmine's fail is already installed on the
// window.
if (!(/** @type {?} */ (globalThis)['fail'])) {
  /**
   * @const
 ...
 * @param {function():string} lazyFailureMessage
 * @suppress {lintChecks} Pre-existing usage of var
 */
var _assert = function(comment, booleanValue, lazyFailureMessage) {
  'use strict';
  ...
  if (adapter) {
    adapter['assertWithMessage'](
        booleanValue,
        goog.testing.JsUnitException.generateMessage(
            comment, lazyFailureMessage()));
  ...
  if (!booleanValue) {
    goog.testing.asserts.raiseException(comment, lazyFailureMessage());
  }
};
...
goog.testing.asserts.assertTrue = function(a, opt_b) {
  ...
  _assert(
      ...
      () => 'Bad argument to assertTrue(boolean): ' +
          _displayStringForValue(booleanValue));
  _assert(
      comment, booleanValue, () => 'Call to assertTrue(boolean) with false');
};
/** @const */
...
goog.testing.asserts.assertFalse = function(a, opt_b) {
  ...
  _assert(
      ...
      () => 'Bad argument to assertFalse(boolean): ' +
          _displayStringForValue(booleanValue));
  _assert(
      comment, !booleanValue, () => 'Call to assertFalse(boolean) with true');
};
/** @const */
...
goog.testing.asserts.assertNotUndefined = function(a, opt_b) {
  ...
  _assert(
      commentArg(1, arguments), aVar !== JSUNIT_UNDEFINED_VALUE,
      () => 'Expected not to be ' +
          _displayStringForValue(JSUNIT_UNDEFINED_VALUE));
};
/** @const */
...
goog.testing.asserts.assertSameElements = function(a, b, opt_c) {
  ...
  // TODO(rcarver): It would be great to show only the difference
...
goog.testing.asserts.assertHashEquals = function(a, b, opt_c) {
  ...
  for (var key in var1) {
    ...
    _assert(
        message, var1[key] == var2[key],
        () => 'Value for key ' + key + ' mismatch - expected = ' + var1[key] +
            ', actual = ' + var2[key]);
  }
...
goog.testing.asserts.contains_ = function(container, contained) {
  'use strict';
  // TODO(rcarver): Can we check for container.contains as well?
